package elastic.AES_EncryptionPipelined

import chisel3._
import chisel3.util._
import elastic.AES_EncryptionFSM
import elastic.AES_EncryptionFSM.{AES_FinalOperation, AES_GetNewKey, AES_InitialOperation, AES_MixColumn, AES_MixColumns, AES_ProcessingElement, AES_RotWord, AES_ShiftRows, AES_SubBytes, AES_SubWord}
...
class AES_GetNewKey(round: Int) extends Module {
  val io = IO(new Bundle {
    val out = new AES_EncryptionFSM.AES_DataInterPE
    val in = Flipped(out)
  })
...
class AES_SubWord extends Module {
  ...
  val PEs = for(i <- 0 until 4) yield {
    val pe = Module(new AES_EncryptionFSM.AES_S)
