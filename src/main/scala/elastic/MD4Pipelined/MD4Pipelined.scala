...
// Fully pipelined MD4 algorithm, it produces data every clock cycle. Several PEs are connected by chain
...
class MD4Pipelined extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val hash = new DecoupledIO(UInt(128.W)) // result of hashing this block of data
  })
...
class MD4FinalAddition extends Module {
  ...
  */
  ...
  val input = RegInit(0.U.asTypeOf(io.in.bits)) // this register contains actual data and should be used for computations
  val valid = RegInit(false.B) // register to show that stored data is valid
  when(enable) { // update values when enable
    input := io.in.bits
    valid := io.in.valid
  }
