...
import elastic.AES_PipelinedElasticBuffer._
import chisel3.tester.RawTester.test
import chisel3.tester._
import chisel3.experimental.BundleLiterals._

// Test for module AES_PipelinedElasticBuffer in decryption mode. Generates waveforms, verilog code and fir file
object TestAES_DecryptionPipelinedElasticBuffer extends App {
  def module = new AES_PipelinedElasticBuffer(encrypt = false) // define calling module for convenience, encrypt = false => decryption
  val name = "AES_DecryptionPipelinedElasticBuffer" // name of output files
  ...
  chisel3.iotesters.Driver.execute(
    ...
    () => module) { c => new TestBenchAES_DecryptionPipelinedElasticBuffer(c)}
...
class TestBenchAES_DecryptionPipelinedElasticBuffer(dut: AES_PipelinedElasticBuffer) extends PeekPokeTester(dut) {
