...
// Test for module DES_PipelinedElasticBuffer in decryption mode. Generates waveforms, verilog code and fir file
object TestDES_DecryptionPipelinedElasticBuffer extends App {
  def module = new DES_PipelinedElasticBuffer(encrypt = false) // define calling module for convenience, encrypt = false => decryption
  val name = "DES_DecryptionPipelinedElasticBuffer" // name of output files
  ...
  chisel3.iotesters.Driver.execute(
    ...
    () => module) { c => new TestBenchDES_DecryptionPipelinedElasticBuffer(c)}
...
class TestBenchDES_DecryptionPipelinedElasticBuffer(dut: DES_PipelinedElasticBuffer) extends PeekPokeTester(dut) {
  step(10)
