...
object TestDES_PipelinedElasticBuffer extends App {
  def module = new DES_EncryptionPipelinedElasticBuffer
  ...
  chisel3.iotesters.Driver.execute(
    ...
    () => module) { c => new TestBenchDES_PipelinedElasticBuffer(c)}
...
class TestBenchDES_PipelinedElasticBuffer(dut: DES_EncryptionPipelinedElasticBuffer) extends PeekPokeTester(dut) {
  step(10)
  poke(dut.io.encrypted.ready, true.B)
