...
import elastic.dotProduct.DotProductDoubleSync

object TestDotProductDoubleSync extends App {
  def module = new DotProductDoubleSync(N = 2)
  val name = "DotProductDoubleSync"
  val dirName = "tested/" + name

  println("[{(Running test bench "+name+")}]")
  chisel3.iotesters.Driver.execute(
    Array("-o", name, "--generate-vcd-output", "on", "--target-dir", dirName),
    () => module) { c => new TestBenchDotProductDoubleSync(c)}
...
class TestBenchDotProductDoubleSync(dut: DotProductDoubleSync) extends PeekPokeTester(dut) {
