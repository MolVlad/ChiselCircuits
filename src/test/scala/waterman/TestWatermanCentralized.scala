...
object TestWatermanCentralized extends App {
//  def module = new WatermanCentralized(rowsNumber = 4, columnsNumber = 4)
  def module = new WatermanCentralized(rowsNumber = 15, columnsNumber = 21)
  val name = "WatermanCentralized"
  val dirName = "tested/" + name

  println("[{(Running test bench)}]")
  chisel3.iotesters.Driver.execute(
    Array("-o", name, "--generate-vcd-output", "on", "--target-dir", dirName),
    () => module) { c => new TestBenchWatermanCentralizedHard(c)}
...
class TestBenchWatermanCentralizedSimple(dut: WatermanCentralized) extends PeekPokeTester(dut) {
...
class TestBenchWatermanCentralizedHard(dut: WatermanCentralized) extends PeekPokeTester(dut) {
  for (i <- 0 until 15) {
